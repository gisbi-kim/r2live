...
// #define COV_ACC_NOISE_DIAG 1e-6
// #define COV_GYRO_NOISE_DIAG 1e-6
...
void ImuProcess::lic_state_propagate(const MeasureGroup &meas, StatesGroup &state_inout) {
   ...
   // if(end_pose_dt > 0)
   // {
   //   cout << ANSI_COLOR_GREEN_BOLD << "End_pose_dt > 0 " << ANSI_COLOR_RESET << endl;
   // }
   // cout << ANSI_COLOR_GREEN_BOLD << "End_pose_dt =  " << end_pose_dt << ANSI_COLOR_RESET << endl;
   ...
   if ((state_in.pos_end - state_inout.pos_end).norm() > 1.0) {
      ...
      for (int i = 0; i < 10; i++) {
         cout << __FILE__ << ", " << __LINE__ << ", check_in_out_state fail !!!! "
              << state_in.pos_end.transpose() << " | " << state_inout.pos_end.transpose() << endl;
      }
...
StatesGroup ImuProcess::imu_preintegration_fast_lio(const StatesGroup &state_in,
   ...
   Eigen::Vector3d acc_imu(0, 0, 0), angvel_avr(0, 0, 0), acc_avr(0, 0, 0), vel_imu(0, 0, 0),
       pos_imu(0, 0, 0);
   ...
   // printf("IMU start_time = %.5f, end_time = %.5f, state_update_time = %.5f, start_delta =
   // %.5f\r\n", v_imu.front()->header.stamp.toSec() - g_lidar_star_tim,
   ...
   for (auto it_imu = v_imu.begin(); it_imu != (v_imu.end() - 1); it_imu++) {
      ...
      cov_omega_diag =
          Eigen::Vector3d(COV_OMEGA_NOISE_DIAG, COV_OMEGA_NOISE_DIAG, COV_OMEGA_NOISE_DIAG)
              .asDiagonal();
      cov_acc_diag =
          Eigen::Vector3d(COV_ACC_NOISE_DIAG, COV_ACC_NOISE_DIAG, COV_ACC_NOISE_DIAG).asDiagonal();
      cov_gyr_diag = Eigen::Vector3d(COV_GYRO_NOISE_DIAG, COV_GYRO_NOISE_DIAG, COV_GYRO_NOISE_DIAG)
                         .asDiagonal();
      ...
      // printf("Tim [%.5f]: ", head->header.stamp.toSec() - g_lidar_star_tim);
   ...
   // cout <<__FILE__ << ", " << __LINE__ <<" ,diagnose lio_state = " << std::setprecision(2)
   // <<(state_inout - StatesGroup()).transpose() << endl;
   ...
   cout << "Vins_pre = " << imu_integration->delta_v.transpose()
        << " | Lio_pre = " << state_inout.vel_end.transpose()
   ...
   // imu_integration->get_lio_cov(state_inout.cov);

   // cout <<__FILE__ << ", " << __LINE__ <<" ,diagnose lio_state = " << std::setprecision(2)
   // <<(state_inout - StatesGroup()).transpose() << endl;

   // cout << "Preintegration State diff = " << std::setprecision(2) << (state_inout -
   // state_in).head<15>().transpose()
   //      << endl;
...
void ImuProcess::lic_point_cloud_undistort(const MeasureGroup &meas,
   ...
   std::sort(pcl_out.points.begin(), pcl_out.points.end(), time_list);
   const double &pcl_end_time = pcl_beg_time + pcl_out.points.back().curvature / double(1000);
   /*std::cout << "[ IMU Process ]: Process lidar from " << pcl_beg_time - g_lidar_star_tim << " to
      " << pcl_end_time- g_lidar_star_tim << ", "
             << meas.imu.size() << " imu msgs from " << imu_beg_time- g_lidar_star_tim << " to " <<
      imu_end_time- g_lidar_star_tim
             << ", last tim: " << state_inout.last_update_time- g_lidar_star_tim << std::endl;
   */
   /*** Initialize IMU pose ***/
   IMU_pose.clear();
   // IMUpose.push_back(set_pose6d(0.0, Zero3d, Zero3d, state.vel_end, state.pos_end,
   // state.rot_end));
   IMU_pose.push_back(set_pose6d(0.0, acc_s_last, angvel_last, state_inout.vel_end,
                                 state_inout.pos_end, state_inout.rot_end));
...
// "<<acc_avr.transpose()<<std::endl;
   ...
   std::cout << "[ IMU Process ]: vel " << state_inout.vel_end.transpose() << " pos "
             << state_inout.pos_end.transpose() << " ba" << state_inout.bias_a.transpose() << " bg "
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup &stat,
   ...
   if (imu_need_init_) {
      ...
      if (init_iter_num > MAX_INI_COUNT) {
         ...
         ROS_INFO(
             "IMU Initials: Gravity: %.4f %.4f %.4f; state.bias_g: %.4f %.4f %.4f; acc covarience: "
             "%.8f %.8f %.8f; gry covarience: %.8f %.8f %.8f",
             stat.gravity[0], stat.gravity[1], stat.gravity[2], stat.bias_g[0], stat.bias_g[1],
             stat.bias_g[2], cov_acc[0], cov_acc[1], cov_acc[2], cov_gyr[0], cov_gyr[1],
             cov_gyr[2]);
      }
