...
inline Eigen::SparseMatrix<double> schur_complement_woodbury_matrix(
    Eigen::SparseMatrix<double> &mat_A, Eigen::SparseMatrix<double> &mat_U,
    Eigen::SparseMatrix<double> &mat_C_inv, Eigen::SparseMatrix<double> &mat_V) {
   // https://en.wikipedia.org/wiki/Woodbury_matrix_identity
   Eigen::SparseMatrix<double> mat_A_inv =
       mat_A.toDense().completeOrthogonalDecomposition().pseudoInverse().sparseView();
   Eigen::SparseMatrix<double> mat_mid_inv = (mat_C_inv + mat_V * mat_A_inv * mat_U)
                                                 .toDense()
...
inline Eigen::SparseMatrix<double> sparse_schur_solver(const Eigen::SparseMatrix<double> &mat_H,
                                                       const Eigen::SparseMatrix<double> &vec_JtX,
                                                       int dense_block_size, int solver_type = 0) {
   ...
   // cout << "Block size = " << mat_H.rows() << " X " << mat_H.cols() << ", dense_blk_size = " <<
   // dense_block_size << endl;

   Eigen::SparseMatrix<double> mat_B, mat_E, mat_Et, mat_C, mat_v, mat_w, mat_C_inv, mat_I, mat_S,
       mat_S_inv, mat_E_C_inv;
...
Start_mat_c:
   ...
   if (if_mat_C_invertable) {
      ...
      if (solver.info() == Eigen::Success) {
         ...
         mat_dy = mat_S.toDense()
                      .completeOrthogonalDecomposition()
                      .solve((mat_v - mat_E_C_inv * mat_w).toDense())
                      .sparseView();
      }
   } else {
      cout << ANSI_COLOR_CYAN_BOLD << "Unstable update, perfrom schur_complement_woodbury_matrix"
...
struct LM_trust_region_strategy {
  public:
   ...
   std::vector<double> cost_history, model_cost_changes_history, real_changes_history,
       step_quality_history, radius_history;
   ...
   inline Eigen::SparseMatrix<double> compute_step(Eigen::SparseMatrix<double> &jacobian,
                                                   Eigen::SparseMatrix<double> &residuals,
                                                   int dense_block_size = 0) {
      ...
      if (cost_history.size() && model_cost_changes_history.size()) {
         ...
         radius_ = 1e-6 * sqrt(std::min(std::max(hessian.diagonal().maxCoeff(), min_diagonal_),
                                        max_diagonal_));
      }
      ...
      for (int i = 0; i < parameter_size; i++) {
         mat_D.coeffRef(i, i) =
             sqrt(std::min(std::max(hessian.coeff(i, i), min_diagonal_), max_diagonal_) * radius_);
      }
      ...
      if (dense_block_size) {
         ...
         current_step =
             (hessian_temp.toDense().completeOrthogonalDecomposition().solve(-gradient.toDense()))
                 .sparseView();
      }
      ...
      if (model_cost_chanage >= 0) {
         ...
         cout << ANSI_COLOR_RED_BOLD << "Model_cost_chanage = " << model_cost_chanage
              << " is negative, something error here, please check!!!" << endl;
         return current_step;
      }
   }
      ...
      if (cost_history.size()) {
         ...
         for (int i = 0; i < cost_history.size() - 1; i++) {
            cout << "[ " << i << "] " << cost_history[i + 1] << " | "
                 << model_cost_changes_history[i] << " | " << real_changes_history[i] << " | "
                 << step_quality_history[i] << " | " << radius_history[i] << endl;
         }
      }
   }
};
