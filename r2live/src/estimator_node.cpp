...
getMeasurements() {
   ...
   while (true) {
      ...
      if (!(imu_buf.front()->header.stamp.toSec() <
            feature_buf.front()->header.stamp.toSec() + estimator.td)) {
         ROS_WARN("throw img, only should happen at the beginning [%.5f | %.5f] ",
                  imu_buf.front()->header.stamp.toSec(), feature_buf.front()->header.stamp.toSec());
         feature_buf.pop();
         continue;
      }
...
void sync_lio_to_vio(Estimator &estimator) {
   ...
   if (abs(g_camera_lidar_queue.m_last_visual_time - g_lio_state.last_update_time) < 1.0) {
      if (g_lio_state.bias_a.norm() < 0.5 && g_lio_state.bias_g.norm() < 1.0) {
         ...
         estimator.Vs[frame_idx] =
             diff_vins_lio_q.toRotationMatrix().inverse() * g_lio_state.vel_end;
...
void construct_camera_measure(
    int frame_idx, Estimator &estimator,
    std::vector<Eigen::Vector2d, Eigen::aligned_allocator<Eigen::Vector2d>> &reppro_err_vec,
    std::vector<Eigen::Matrix<double, 2, 6, Eigen::RowMajor>,
                Eigen::aligned_allocator<Eigen::Matrix<double, 2, 6, Eigen::RowMajor>>>
        &J_mat_vec) {
   J_mat_vec.clear();
   ...
   for (auto &it_per_id : estimator.f_manager.feature) {
      ...
      if (!(it_per_id.used_num >= 2 && it_per_id.start_frame < WINDOW_SIZE - 1)) continue;
      ...
      for (auto &it_per_frame : it_per_id.feature_per_frame) {
         ...
         if (imu_j == (frame_idx)) {
            ...
            Eigen::Quaterniond Qi(parameters_vec[0][6], parameters_vec[0][3], parameters_vec[0][4],
                                  parameters_vec[0][5]);

            Eigen::Vector3d Pj(parameters_vec[1][0], parameters_vec[1][1], parameters_vec[1][2]);
            Eigen::Quaterniond Qj(parameters_vec[1][6], parameters_vec[1][3], parameters_vec[1][4],
                                  parameters_vec[1][5]);

            Eigen::Vector3d tic(parameters_vec[2][0], parameters_vec[2][1], parameters_vec[2][2]);
            Eigen::Quaterniond qic(parameters_vec[2][6], parameters_vec[2][3], parameters_vec[2][4],
                                   parameters_vec[2][5]);
            ...
            if (0) {
               ...
               f->Evaluate(parameters_vec.data(), residual_vec_old.data(),
                           (double **)jacobian_mat_vec);
               j_mat_old.block(0, 3, 2, 3) = j_mat_tq.block(0, 0, 2, 3);
               j_mat_old.block(0, 0, 2, 3) = j_mat_tq.block(0, 3, 2, 3);
            }

            if (1) {
               visual_imu_measure(pts_i, pts_j, Pi, Qi, Pj, Qj, tic, qic, inverse_depth,
                                  residual_vec, j_mat);
            }
   ...
   while (true) {
      ...
      for (auto &measurement : measurements) {
         ...
         if (estimator.m_fast_lio_instance != nullptr) {
            ...
            lock_lio(estimator);
            t_s.tic();
            double camera_LiDAR_tim_diff = img_msg->header.stamp.toSec() +
                                           g_camera_lidar_queue.m_camera_imu_td -
                                           g_lio_state.last_update_time;
            *p_imu = *(estimator.m_fast_lio_instance->m_imu_process);
         }

         if ((g_camera_lidar_queue.m_if_lidar_can_start == true) &&
             (g_camera_lidar_queue.m_lidar_drag_cam_tim >= 0)) {
            m_state.lock();
         ...
         if (imu_queue.size()) {
            ...
            if (g_camera_lidar_queue.m_if_have_lidar_data &&
               ...
               *p_imu = *(estimator.m_fast_lio_instance->m_imu_process);
               state_aft_integration = p_imu->imu_preintegration(
                   g_lio_state, imu_queue, 0,
                   cam_update_tim - imu_queue.back()->header.stamp.toSec());
               estimator.m_lio_state_prediction_vec[WINDOW_SIZE] = state_aft_integration;

               diff_vins_lio_q =
                   eigen_q(estimator.Rs[WINDOW_SIZE].transpose() * state_aft_integration.rot_end);
               ...
               if ((start_dt > -0.02) && (fabs(end_dt) < 0.02)) {
                  ...
                  cout << "Start time = " << std::setprecision(8)
                       << imu_queue.front()->header.stamp.toSec() -
                              estimator.m_fast_lio_instance->first_lidar_time
                       << endl;
                  cout << "Final time = " << std::setprecision(8)
                       << cam_update_tim - estimator.m_fast_lio_instance->first_lidar_time << endl;
         ...
         if (esikf_update_valid && (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR) &&
             (g_lio_state.last_update_time - g_camera_lidar_queue.m_visual_init_time >
              g_camera_lidar_queue.m_lidar_drag_cam_tim)) {
            estimator.vector2double();
            ...
            for (int iter_time = 0; iter_time < 2; iter_time++) {
               Eigen::Quaterniond q_pose_last =
                   Eigen::Quaterniond(state_aft_integration.rot_end * diff_vins_lio_q.inverse());
               ...
               auto num_eff_residuals = reppro_err_vec.size();  // aka features_correspondences
               if (num_eff_residuals < minmum_number_of_camera_res) {
                  cout << "Size of reppro_err_vec: " << num_eff_residuals << endl;
                  break;
               }

               // TODO: Add camera residual here
               cout << "number of effective [visual] residuals: " << num_eff_residuals << endl;

               meas_vec.resize(2 * num_eff_residuals, 1);
               Hsub.resize(2 * num_eff_residuals, 6);
               for (int residual_idx = 0; residual_idx < reppro_err_vec.size(); residual_idx++) {
                  ...
                  // J_mat_vec[residual_idx].block(0,0,2,3) =
                  // J_mat_vec[residual_idx].block(0,0,2,3)
                  // * extrinsic_vins_lio_q.toRotationMatrix().transpose();
               ...
               if (use_k_fast) {
                  faster_lio::Timer::Evaluate(
                      [&]() {
                         ...
                         K_1.setZero();
                         K_1 = (H_T_H + (state_aft_integration.cov * CAM_MEASUREMENT_COV).inverse())
                                   .inverse();

                         K.resize(DIM_OF_STATES, num_eff_residuals);
                         K = K_1.block<DIM_OF_STATES, 6>(0, 0) * Hsub_T;
                      },
                      "Calc Visual KalmanGain (fast ver)");
               } else {
                  faster_lio::Timer::Evaluate(
                      [&]() {
                         auto P = state_aft_integration.cov;

                         auto H_height = 2 * num_eff_residuals;

                         Eigen::Matrix<double, -1, -1> HPHT;
                         HPHT.resize(H_height, H_height);
                         HPHT = Hsub * P * Hsub_T;

                         Eigen::Matrix<double, -1, -1> R;
                         R.resize(H_height, H_height);
                         R = Eigen::MatrixXd::Identity(H_height, H_height);
                         R *= CAM_MEASUREMENT_COV;

                         K.resize(DIM_OF_STATES, num_eff_residuals);
                         K = P * Hsub_T * (HPHT + R).inverse();
                      },
                      "Calc Visual KalmanGain (slow ver)");
               }
            ...
            if (reppro_err_vec.size() >= minmum_number_of_camera_res) {
               ...
               if ((rot_add.norm() * 57.3 < 3) && (t_add.norm() < 0.5) &&
                   (mean_reprojection_error < 1.0)) {
                  g_lio_state = state_aft_integration;
                  eigen_q q_I = eigen_q(1.0, 0, 0, 0);
                  double angular_diff =
                      eigen_q(g_lio_state.rot_end.transpose() * state_before_esikf.rot_end)
                          .angularDistance(q_I) *
                      57.3;
                  ...
                  // g_lio_state.last_update_time + 0.02;
                  // // Unblock lio process, forward 80 ms
               }
            }
         }
         ...
         if (g_camera_lidar_queue.m_if_have_lidar_data) {
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && esikf_update_valid) {
               ...
               if ((g_lio_state.last_update_time - g_camera_lidar_queue.m_visual_init_time >
                    g_camera_lidar_queue.m_lidar_drag_cam_tim)) {
                  StatesGroup state_before_esikf = g_lio_state;
                  ...
                  g_lio_state.vel_end =
                      diff_vins_lio_q.toRotationMatrix() * estimator.Vs[WINDOW_SIZE];
                  g_lio_state.cov = state_aft_integration.cov;

                  Eigen::Matrix3d temp_R =
                      estimator.Rs[WINDOW_SIZE] * diff_vins_lio_q.toRotationMatrix();
                  Eigen::Vector3d temp_T = estimator.Ps[WINDOW_SIZE] + diff_vins_lio_t;
                  eigen_q q_I = eigen_q(1.0, 0, 0, 0);
                  double angular_diff = eigen_q(temp_R.transpose() * state_before_esikf.rot_end)
                                            .angularDistance(q_I) *
                                        57.3;
...
int main(int argc, char **argv) {
   ...
   get_ros_parameter<int>(nh, "/if_write_to_bag", g_camera_lidar_queue.m_if_write_res_to_bag,
                          false);
   ...
   ros::Subscriber sub_imu =
       nh.subscribe(IMU_TOPIC, 20000, imu_callback, ros::TransportHints().tcpNoDelay());
   ros::Subscriber sub_image = nh.subscribe("/feature_tracker/feature", 20000, feature_callback,
                                            ros::TransportHints().tcpNoDelay());
   ros::Subscriber sub_restart = nh.subscribe("/feature_tracker/restart", 20000, restart_callback,
                                              ros::TransportHints().tcpNoDelay());
   ros::Subscriber sub_relo_points =
       nh.subscribe("/pose_graph/match_points", 20000, relocalization_callback,
                    ros::TransportHints().tcpNoDelay());
